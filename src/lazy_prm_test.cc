...
BOOST_AUTO_TEST_CASE(build) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 10);
...
BOOST_AUTO_TEST_CASE(buildh) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 10);
  BOOST_CHECK_EQUAL(lazy_prm.BuildTree("lazy_trivialh"), true);
  auto end_t = std::chrono::steady_clock::now();
  auto duration = end_t - start_t;
  std::cout << "Elapsed time: " <<
    std::chrono::duration <double, std::milli> (duration).count() << " ms" <<
    std::endl;
  lazy_prm.GeneratePath("lazy_rdk_trivialh.py");
}
...
BOOST_AUTO_TEST_CASE(build1h) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 20);
  BOOST_CHECK_EQUAL(lazy_prm.BuildTree("lazy_easyh"), true);
  auto end_t = std::chrono::steady_clock::now();
  auto duration = end_t - start_t;
  std::cout << "Elapsed time: " <<
    std::chrono::duration <double, std::milli> (duration).count() << " ms" <<
    std::endl;
  lazy_prm.GeneratePath("lazy_rdk_easyh.py");
}
...
BOOST_AUTO_TEST_CASE(build2) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 60);
...
BOOST_AUTO_TEST_CASE(build2h) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 60);
  BOOST_CHECK_EQUAL(lazy_prm.BuildTree("lazy_hardh"), true);
  auto end_t = std::chrono::steady_clock::now();
  auto duration = end_t - start_t;
  std::cout << "Elapsed time: " <<
    std::chrono::duration <double, std::milli> (duration).count() << " ms" <<
    std::endl;
  lazy_prm.GeneratePath("lazy_rdk_hardh.py");
}
...
BOOST_AUTO_TEST_CASE(build3h) {
  ...
  LazyPrm lazy_prm (pqp.release(), start, end, 60);
  BOOST_CHECK_EQUAL(lazy_prm.BuildTree("lazy_hard2h"), true);
  auto end_t = std::chrono::steady_clock::now();
  auto duration = end_t - start_t;
  std::cout << "Elapsed time: " <<
    std::chrono::duration <double, std::milli> (duration).count() << " ms" <<
    std::endl;
  lazy_prm.GeneratePath("lazy_rdk_hard2h.py");
}
